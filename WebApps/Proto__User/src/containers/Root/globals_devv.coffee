...
# could import some limited set of glmat if needed
# also may could cut down on our lodash package with custom build only used functions.
...
primus.on 'data', (data) ->
    c 'have data', data
# todo : make a line below which can be copy pasted from dev version to staging version
setInterval =>
    primus.write
